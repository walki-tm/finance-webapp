...
export default function Dashboard({ state, year, onSelectMain, detailMain, addTx }) {
  ...
  const budgetTotals = useMemo(() => {
    const map = state.budgets[year] || {};
    const totals = { income: 0, expense: 0, debt: 0, saving: 0 };
    for (const k in map) { const [m] = k.split(':'); totals[m] += map[k]; }
    return totals;
  }, [state.budgets, year]);

  const donutData = MAIN_CATS.map(mc => ({
    name: mc.name, key: mc.key, value: sums[mc.key], color: mc.color, budget: budgetTotals[mc.key]
  }));
    ...
    <div className="space-y-6">
      ...
      {detailMain && (
        <DetailPanel
          ...
          state={state}
          year={year}
          color={MAIN_CATS.find(m => m.key === detailMain).color}
          addTx={addTx}
        />
      )}
              ...
              <AreaChart data={monthly}>
                <defs>
                  <linearGradient id="g-expense" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#63C4FF" stopOpacity={0.6} />
                    <stop offset="95%" stopColor="#63C4FF" stopOpacity={0} />
                  </linearGradient>
                  <linearGradient id="g-income" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#56E2C6" stopOpacity={0.6} />
                    <stop offset="95%" stopColor="#56E2C6" stopOpacity={0} />
                  </linearGradient>
                </defs>
...
function DetailPanel({ mainKey, state, year, color, addTx }) {
  ...
  function submitQuick(e) {
    e?.preventDefault?.();
    if (!qaSub) return;
    addTx({ main: mainKey, sub: qaSub, amount: Number(qaAmt), date: qaDate, note: qaNote });
    ...
    <Card>
      <CardContent>
        <div className="flex items-center justify-between mb-3">
          ...
          <Button size="sm" onClick={() => setQaOpen(v => !v)} className="rounded-xl">
            <Plus className="h-4 w-4 mr-2" />Aggiungi transazione
          </Button>
        </div>
        ...
        {qaOpen && (
          <div className="mb-4 grid md:grid-cols-4 gap-2 bg-slate-50 dark:bg-slate-900 border border-slate-200/30 rounded-xl p-3">
            <div>
              ...
              >
                {subcats.map(s => <option key={s.name} value={s.name}>{s.name}</option>)}
              </select>
            </div>
            <div>
            ...
            <tbody>
              {rows.map(r => (
                <tr key={r.sub} className="border-t border-slate-200/10">
                  ...
                  <td className={r.delta < 0 ? 'p-2 text-right text-red-500' : 'p-2 text-right text-emerald-500'}>
                    {nice(r.delta)}
                  </td>
